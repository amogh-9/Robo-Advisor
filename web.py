...
def plot_pie_chart(investments):
    fig, ax = plt.subplots()
...
def result():
    ...
    # Generate plot for portfolio metrics
    plot_url = plot_portfolios(expected_returns_global, expected_risk_global, expected_sharpe_ratio_global, optimal_portfolio)
    
    # Generate pie chart for investments distribution
    pie_chart_url = plot_pie_chart(investments)
    
    return render_template('result.html', amount=amount, period=period, risk=risk, plot_url=plot_url, pie_chart_url=pie_chart_url, optimal_portfolio=optimal_portfolio, investments=investments)
